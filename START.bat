...
SET "GOOGLE_DRIVE_DATE="
FOR /F "usebackq delims=" %%D IN (`powershell.exe -NoProfile -Command "Get-Date -Format yyyyMMdd"`) DO SET "GOOGLE_DRIVE_DATE=%%D"
...
SET "TEST_RUN_MARKER=%TEMP%\ML_PostProcess_run_%RANDOM%_%RANDOM%.tmp"
...
SET "LOG_ARCHIVE_CATEGORY=Robocal"
IF /I "%TYPE%"=="Post" SET "LOG_ARCHIVE_CATEGORY=PostProcess"
IF /I "%TYPE%"=="PostProcess" SET "LOG_ARCHIVE_CATEGORY=PostProcess"
SET "LOG_ARCHIVE_DESTINATION=%LOG_ARCHIVE_ROOT%\%LOG_IDENTIFIER%\%LOG_ARCHIVE_CATEGORY%"
SET "LOG_ARCHIVE_ROBOCOPY_LOG=%TEMP%\ML_PostProces_archive_%RANDOM%_%RANDOM%.log"
...
FOR /F "usebackq skip=1 tokens=1" %%A IN ("%TEMP%\ML_PostProces_adb_devices.tmp") DO SET "ADB_DEVICE_CONNECTED=TRUE"
...
CALL "%GOOGLE_DRIVE_UPLOAD_BAT%" "%GOOGLE_DRIVE_SOURCE%" "%GOOGLE_DRIVE_URL%" "gdrive" "%GOOGLE_DRIVE_DATE%/%LOG_IDENTIFIER%"
